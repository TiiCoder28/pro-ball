...
function limitSpeed(body, max) { const speed = Math.hypot(body.vx, body.vy); if (speed > max) { body.vx = (body.vx / speed) * max; body.vy = (body.vy / speed) * max; } }
function opposite(team) { if (team === 'red') return 'blue'; if (team === 'blue') return 'red'; return 'red'; }
function activePlayers(state, team) { return Object.values(state.players).filter((player) => player.team === team); }
...
function givePossession(state, team, x, y) {
  ...
  const player = players.reduce((best, candidate) => Math.hypot(candidate.x - ball.x, candidate.y - ball.y) < Math.hypot(best.x - ball.x, best.y - ball.y) ? candidate : best, players[0]);
...
function collidePlayers(players) {
  for (let i = 0; i < players.length; i++) {
    for (let j = i + 1; j < players.length; j++) {
      ...
      const dx = b.x - a.x, dy = b.y - a.y, dist = Math.hypot(dx, dy) || 1, min = FIELD.playerR * 2;
...
function collideBallPlayer(state, player) {
  const ball = state.ball;
  const dx = ball.x - player.x, dy = ball.y - player.y, dist = Math.hypot(dx, dy) || 1, min = FIELD.playerR + FIELD.ballR;
  if (dist >= min) return;
...
export function step(state, dt = 1 / 30) {
  ...
  ball.x += ball.vx; ball.y += ball.vy; ball.vx *= 0.982; ball.vy *= 0.982;
  ...
  if (ball.y < -FIELD.ballR) { givePossession(state, awardedTeam, ball.x, FIELD.ballR + 36); return state; }
  if (ball.y > FIELD.h + FIELD.ballR) { givePossession(state, awardedTeam, ball.x, FIELD.h - FIELD.ballR - 36); return state; }
  const inGoal = ball.y > FIELD.h / 2 - FIELD.goalH / 2 && ball.y < FIELD.h / 2 + FIELD.goalH / 2;
  if (ball.x < -FIELD.ballR) { if (inGoal) { state.blue += 1; resetPositions(state); } else { givePossession(state, awardedTeam, FIELD.ballR + 36, ball.y); } return state; }
  if (ball.x > FIELD.w + FIELD.ballR) { if (inGoal) { state.red += 1; resetPositions(state); } else { givePossession(state, awardedTeam, FIELD.w - FIELD.ballR - 36, ball.y); } return state; }
  return state;
}
...
export function resetPositions(state) {
  ...
  for (const player of Object.values(state.players)) {
    ...
    player.x = position.x; player.y = position.y; player.vx = 0; player.vy = 0;
  }
}
